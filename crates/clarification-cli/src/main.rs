...
fn main() {
    ...
    match args[1].as_str() {
        ...
        "ai" => {
            ...
            let face_weight = value(&args, "--face-weight", 0.50).clamp(0.0, 1.0);
            let face_weight_value = face_weight.to_string();
            let eye_blend = value(&args, "--eye-blend", 0.65).clamp(0.0, 1.0);
            let eye_blend_value = eye_blend.to_string();
